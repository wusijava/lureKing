<template>
    <div class="box">
        <van-notice-bar
                left-icon="volume-o"
                text="按月统计当月鱼获数量:统计本月鱼获总数量，并展示每个鱼种条数分布情况！"
        />
        ...
        <div class="content">
            ...
            <div class="list" v-for="item in list" :key="item.id" style="color: #cf2d28">
                <van-cell-group >
                    <van-cell title="月份" :value="item.month" style="color: #cf2d28" size="big" />
                    <van-cell title="数量" :value="item.num" style="color: #cf2d28"/>
                    <div class="list" v-for="item in item.fishCount" :key="item.id">
                    <div ><van-cell title="鱼种" :value="item.fishName" />
                        <van-cell title="数量" :value="item.num" /></div>
                    </div>
                </van-cell-group>
                </div>
           ...
           <!-- <van-button class="button" @click="query" type="primary" size="large" >查询数量</van-button>-->
...
<script>
    import {monthCount,receiveWork} from "../../api/order";
    import Vue from 'vue';
    ...
    export default {
        ...
        methods: {
            ...
            getList: async function(cp,c) {
                ...
                const result = await monthCount(params);
                this.$toast.clear();
