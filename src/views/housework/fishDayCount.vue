<template>
    <div class="box">
        <van-notice-bar
                left-icon="volume-o"
                text="把已有的鱼获按鱼种分类，查询距离上次中鱼的时间间隔！"
        />



        <div class="content">
            <van-empty image="search" description="暂无记录信息" v-show="showEmpty"/>
            <div class="list" v-for="item in list" :key="item.id" >
                <van-cell-group >
                    <van-cell title="鱼种" :value="item.fish" />
                    <van-cell title="上一条中鱼时间" :value="item.lastDate" />
                    <van-cell title="距离天数" :value="item.day" />
                </van-cell-group>
                </div>
...
<script>
    import {fishDayCount,receiveWork} from "../../api/order";
    import Vue from 'vue';
    ...
    export default {
        ...
        methods: {
            ...
            getList: async function(cp,c) {
                ...
                params.type= 'month'
                const result = await fishDayCount(params);
                this.$toast.clear();
