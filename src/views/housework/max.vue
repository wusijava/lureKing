<template>
    <div class="box">
        <van-notice-bar
                left-icon="volume-o"
                text="将本系统所有鱼获进行统计分析，展示每种鱼获的最大个体信息！鱼种，长度，重量，中鱼时间，地址等信息！"
        />



        <div class="content">
            <van-empty image="search" description="暂无最大个体信息" v-show="showEmpty"/>
            <div class="list" v-for="item in list" :key="item.id" >
                <van-cell-group >
                    <van-cell title="鱼种" :value="item.name" />
                    <van-cell title="长度(厘米)" :value="item.length" />
                    <van-cell title="重量(斤)" :value="item.weight" />
                    <van-cell title="时间" :value="item.time" />
                    <van-cell title="中鱼地址" :value="item.address" />
                    <van-cell title="中鱼大师" :value="item.userName" />
                    <van-button type="info" plain hairline round size="small" class="btn-small" @click="showImage(item.url)" style="margin-left: 150px;margin-bottom: 20px">
                        鱼获展示
                    </van-button>
                </van-cell-group>
                </div>
        ...
        <van-popup v-model="showDetails" class="detail" :close-on-click-overlay="false" closeable >
            ...
            <div class="detail-main">
                <img :src=this.urlIamge style="width: 100%;height:100%" v-if="this.urlIamge!=null&&this.urlIamge!=''" >
            </div>
        </van-popup>
    </div>
...
<script>
    import {geti,receiveWork} from "../../api/order";
    import Vue from 'vue';
    ...
    export default {
        ...
        methods: {
            ...
            getList: async function(cp,c) {
                ...
                params.type= 'max'
                const result = await geti(params);
                this.$toast.clear();
