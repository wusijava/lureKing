<template>
    <div class="box">
        <van-notice-bar
                left-icon="volume-o"
                text="把本年度已有数据的大师进行打龟率排行，谁是打龟王，一目了然!只统计当前年份！"
        />



        <div class="content">
            <van-empty image="search" description="暂无本年龟王" v-show="showEmpty"/>
            <div class="list" v-for="item in list" :key="item.id" >
                <van-cell-group >
                    <van-cell title="名次" :value="item.index" />
                    <van-cell title="姓名" :value="item.name" />
                    <van-cell title="龟率" :value="item.rateStr" />
                </van-cell-group>
                </div>
...
<script>
    import {guiWang,receiveWork} from "../../api/order";
    import Vue from 'vue';
    ...
    export default {
        ...
        methods: {
            ...
            getList: async function(cp,c) {
                let params = {};
                params.type= 'year'
                const result = await guiWang(params);
                this.$toast.clear();
