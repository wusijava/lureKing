<template>
    <div class="box">
        <van-notice-bar
                left-icon="volume-o"
                text="店铺排班计划表!打工人,打工魂,打工人干饭得用盆~~~奥力给!"
        />
        ...
        <div class="content">
            <div class="list" v-for="item in list" :key="item.id" style="border: #ee0a24 solid 1px">
                <h5 style="text-align: center;color: red;font-weight: bolder">日期:{{item.date}}&nbsp;</h5>
                    <div class="list2" v-for="item2 in item.planList" :key="item2.id" v-if="item.planList!=null">
                        <van-cell-group :title="item.createTime">
                            <van-cell :title="item2.name" :value="item2.classPlanDesc" />
                        </van-cell-group>
                    </div>
                </div>
    ...
    export default {
        ...
        methods: {
            ...
            getList: async function(cp,c) {
                ...
                const result = await paiBan(params);
                this.$toast.clear();
