       ...
       <!-- <van-dropdown-menu @change.native="changeValue">
        ...
        <div class="content">
            <div class="list" v-for="item in list" :key="item.id" style="border: #ee0a24 solid 1px">
                <h5 style="text-align: center">期数:{{item.term}}&nbsp;开奖时间:{{item.bonusTime}}</h5>
...
<script>
    import {history,buyRecord} from "../../api/order";
    import Vue from 'vue';
        ...
        methods: {
            ...
            getList: async function(cp,c) {
               ...
               /* this.$toast.loading({
                ...
                console.log(result)
                console.log(result.data.data.content.totalPages)
                console.log(result.data.data.totalPages)
                this.$toast.clear();
