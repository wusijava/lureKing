<template>
    <div class="box">
       ...
       <!-- <van-dropdown-menu @change.native="changeValue">
        ...
        <div class="content">
            <h5 style="text-align: center">总佣金:{{this.commission}}元</h5>
            <div class="list" v-for="item in list" :key="item.id" style="border: #ee0a24 solid 1px">
                <h5 style="text-align: center">期数:{{item.term}}&nbsp;订单时间:{{item.ssqList[0].createTime}}</h5>
                    <div class="list2" v-for="item2 in item.ssqList" :key="item2.id" v-if="item.ssqList!=null">
                        ...
                        <van-button round type="warning" style="margin-left: 10px;margin-left: 130px;margin-bottom: 5px" size="large;" v-if="item2.state==0" @click="doJob(item2.id,2)">接受订单</van-button>
                        <van-button round type="info" style="margin-right: 10px;margin-bottom: 5px" size="large" v-if="item2.state==2"  @click="doJob(item2.id,1)">完成购买</van-button>
                    </div>
...
<script>
    import {ssqRecord,buyRecord,changeSsqState} from "../../api/order";
    import Vue from 'vue';
    ...
    export default {
        ...
        methods: {
               ...
               /* this.$toast.loading({
                ...
                if (result.data.code == '20000') {
                    if(result.data.data.length > 0) {
                        ...
                        this.commission=result.data.data[0].commission
            ...
            doJob: async function(id,state){
                let params = {};
                params.id=id
                params.state=state
                const result = await changeSsqState(params);
