<template>
    <div class="box">
        <van-notice-bar
                left-icon="volume-o"
                text="根据历史开奖信息统计每个球出现的次数,默认查最少出现的前八个~"
        />
        ...
        <div class="content">
            <div class="list" v-for="item in list" :key="item.id" >
                <van-cell-group :title="item.createTime">
                    <van-cell title="红球号码" :value="item.key" />
                    <van-cell title="出现次数" :value="item.value" />
                    <div style="text-align: center">
                        <van-button type="danger" v-if="item.isDo=='0'" size="small"  @click="doJob(item.id)" >已处理</van-button>
                    </div>
                </van-cell-group>
                </div>
            <van-stepper v-model="value" integer style="margin-top: 10px;text-align: center" min="1" max="33"/>
            <h6 style="text-align: center">选择查询最少出现红球个数</h6>
            <van-button class="button" @click="getList" type="warning" size="large" >查询</van-button>
            <van-button class="button" @click="back" type="info" size="large" style="margin-top: 20px" >回菜单</van-button>
        </div>
    </div>
</template>

<script>
    import {gaiLv,changeState} from "../../api/order";
    import Vue from 'vue';
    ...
    export default {
        ...
        methods: {
            ...
            getList: async function(cp,c) {
                ...
                const result = await gaiLv(params);
                this.$toast.clear();
