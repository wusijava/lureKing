<template>
    <div class="box">
        <van-notice-bar
                left-icon="volume-o"
                text="根据历史开奖信息统计最久未出现的蓝球,个数可以自定义~"
        />
        ...
        <div class="content">
            <div class="list" v-for="item in list" :key="item.id" >
                <van-cell-group :title="item.createTime">
                    <van-cell title="蓝球号码" :value="item" />
                    <div style="text-align: center">
                        <van-button type="danger" v-if="item.isDo=='0'" size="small"  @click="doJob(item.id)" >已处理</van-button>
                    </div>
                </van-cell-group>
                </div>
            <van-stepper v-model="value" integer style="margin-top: 10px;text-align: center" min="1" max="16"/>
            <h6 style="text-align: center">选择查询最久未出现蓝球个数</h6>
...
<script>
    import {changeState,blueThree} from "../../api/order";
    import Vue from 'vue';
    ...
    export default {
        ...
        methods: {
            ...
            getList: async function(cp,c) {
                ...
                const result = await blueThree(params);
                this.$toast.clear();
